...
def check(buffer):
    ...
    try:
        ...
        result = [SyntaxError(filename, lineno, offset, str(value))]
    else:
    ...
    pep8_check = Checker(lines=contents)
    old_stdout, sys.stdout = sys.stdout, blackhole()
    pep8_check.check_all()
    sys.stdout = old_stdout
    result += [SyntaxError(filename, line[0], line[1], line[2] + line[3]) for line in pep8_check.report._deferred_print]
...
au BufLeave *.py call s:ClearPyflakes()

au BufEnter *.py call s:RunPyflakes()
au InsertLeave *.py call s:RunPyflakes()
au InsertEnter *.py call s:RunPyflakes()
au BufWritePost *.py call s:RunPyflakes()

au CursorHold *.py call s:RunPyflakes()
au CursorHoldI *.py call s:RunPyflakes()

au CursorHold *.py call s:GetPyflakesMessage()
